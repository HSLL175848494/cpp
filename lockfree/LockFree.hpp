...
#define LOCKFREE_CV(mtx, cv, time)                       \
	if (!Wait)                                           \
		return 0;                                        \
	std::unique_lock<std::mutex> uLock(mtx);             \
	cv.wait_for(uLock, std::chrono::milliseconds(time)); \
	if (TimeOut != 0xffffffff)                           \
		TimeOut <<= 1;                                   \
	goto Resume;

#define LOCKFREE_MALLOC                           \
...
#define LOCKFREE_NODE       \
	reValue = pFree->value; \
	free(pFree);

#define LOCKFREE_FREELIST                  \
...
#define LOCKFREE_QUEUE_POP_BULK_LIST           \
	...
	public:
		...
		bool WaitPush(Type &&inValue)
		{
			...
			if (Old.Size == MaxLength)
			{
				LOCKFREE_CV(mtxPush, cvPush, TimeOut);
			}
			LOCKFREE_STACK_PUSH;
			cvPop.notify_all();
			return true;
		...
		bool WaitPop(T &reValue)
		{
			...
			if (!Old.Size)
			{
				LOCKFREE_CV(mtxPop, cvPop, TimeOut);
			}
			LOCKFREE_STACK_POP;
			cvPush.notify_all();
			return true;
