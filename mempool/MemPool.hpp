...
#define Cheak_Free \
	...
	if (--(*pBlock->pRef) == 0)_aligned_free(pBlock->pRef);}
...
#define GetBlock(AllocSize)\
	...
	if (pBlock)\
		...
		pResult=(T*)(i > 1 ? AllocIndex(i - 1, AllocSize, true) : AllocIndex(i, Size, true));\
	...
	public:
		...
		~MemoryPool()//释放内存池(还未free的内存将泄露)
		{
			...
			for (int i = 0; i < 32; i++)//根据引用数目释放栈内存()
			{
				...
				while (pBlock)
				{
					typename Type::type_pBlock pTemp = pBlock->next;
					Cheak_Free
					pBlock = pTemp;
				}
			}
